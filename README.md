...
## 5) تثبيت التطبيق على الموبايل / الكمبيوتر (PWA)
التطبيق دلوقتي Progressive Web App، يعني بيتنصّب زي أي تطبيق عادي:

- **أندرويد (Chrome):** هيظهر زرار "تثبيت التطبيق" تلقائي تحت يمين الشاشة، أو من قائمة المتصفح ⋮ → "Add to Home screen" / "Install app".
- **آيفون/آيباد (Safari):** مفيش زرار تلقائي (قيود آبل)، لازم تدوس على زرار المشاركة (المربع وسهم لفوق) → **"إضافة إلى الشاشة الرئيسية" / Add to Home Screen**.
- **الكمبيوتر (Chrome / Edge):** هيظهر أيقونة تثبيت ⊕ جنب شريط العنوان، أو زرار "تثبيت التطبيق" في الصفحة نفسها.

بعد التثبيت هيفتح في نافذة مستقلة بأيقونة خاصة بيه، بدون شريط عنوان المتصفح، وهيشتغل حتى لو النت بطيء (فيه Service Worker بيعمل cache للواجهة نفسها؛ البيانات لسه محتاجة نت عشان بتيجي من Supabase).

**ملحوظة:** لازم الموقع يكون شغال على HTTPS (GitHub Pages بيدي HTTPS تلقائي)، وده شرط أساسي عشان التثبيت يشتغل.

الملفات الجديدة اللي اتضافت لده: `manifest.json`, `sw.js`, `icon-192.png`, `icon-512.png` — لازم تترفع كلها مع `index.html` و `app-supabase.js` في نفس المجلد بالظبط.
