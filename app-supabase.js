...
const GAS_API = {
  ...
  async loginResident(u, p) {
    const { data, error } = await sb.rpc('login_resident', { p_unit: u, p_password: p });
    ok(error);
    return data;
  },

  async updateProfile(u, n, p, pw) {
    const { data, error } = await sb.rpc('update_profile', { p_unit: u, p_name: n, p_phone: p, p_password: pw || null });
    ok(error);
    return data;
  },

  async getAllResidentsForRoleManagement() {
    const { data, error } = await sb.rpc('get_all_residents_for_role_management');
    ok(error);
    return (data || []).sort((a, b) => unitLocaleCompare(a.unit, b.unit));
  },

  async setUnitRole(unit, newRole) {
    const { error } = await sb.rpc('set_unit_role', { p_unit: unit, p_new_role: newRole });
    ok(error);
    return true;
  },

  async resetUnitPassword(unit, newPassword) {
    const { error } = await sb.rpc('reset_unit_password', { p_unit: unit, p_new_password: newPassword });
    ok(error);
    return true;
  },

  async addNewUnit(unit, name, phone, password, role) {
    const { error } = await sb.rpc('add_new_unit', { p_unit: unit, p_name: name, p_phone: phone, p_password: password, p_role: role });
    ok(error);
    return true;
  },

  async deleteUnit(unit) {
    const { error } = await sb.rpc('delete_unit', { p_unit: unit });
    ok(error);
    return true;
  },
  ...
  async addAnnouncement(t, c) {
    const { error } = await sb.from('announcements').insert({ title: t.trim(), content: c.trim() });
    ok(error);
    return true;
  },
  ...
  async getLatestAnnouncements() {
    const { data, error } = await sb.from('announcements').select('*').order('created_at', { ascending: false }).limit(5);
    ok(error);
  ...
  // ----- surveys ---------------------------------------------------------------
  async addSurvey(q, o) {
    const { error } = await sb.from('surveys').insert({ question: q.trim(), options: o, responses: [] });
    ok(error);
    return true;
  },
  ...
  async submitSurveyResponse(id, u, o) {
    const { data, error } = await sb.from('surveys').select('responses').eq('id', id).single();
    ...
    const { error: upErr } = await sb.from('surveys').update({ responses: r }).eq('id', id);
  ...
  async updateSurvey(id, q, o) {
    const { error } = await sb.from('surveys').update({ question: q.trim(), options: o }).eq('id', id);
    return !error;
  },

  // ----- finances ----------------------------------------------------------
  async addTransaction(u, o, p) {
    const { error } = await sb.from('finances').insert({ unit: u.trim(), amount_owed: parseFloat(o) || 0, amount_paid: parseFloat(p) || 0 });
    ok(error);
    return true;
  },
  ...
  async addExpense(desc, amt, fd, fn, mt) {
    ...
    if (fd) {
      ...
      const { error: upErr } = await sb.storage.from('invoices').upload(path, blob, { contentType: mt });
      ok(upErr);
      invoiceUrl = sb.storage.from('invoices').getPublicUrl(path).data.publicUrl;
    }
    const { error } = await sb.from('expenses').insert({ description: desc.trim(), amount: parseFloat(amt) || 0, invoice_url: invoiceUrl });
    ok(error);
    return true;
  },

  async addBulkDues(amt) {
    const { error } = await sb.rpc('add_bulk_dues', { p_amount: parseFloat(amt) || 0 });
    ok(error);
    return true;
  },

  async getFinancesSummary(u) {
    const { data, error } = await sb.rpc('get_finances_summary', { p_unit: u });
    ok(error);
    return data;
  },

  async getDetailedReports() {
    const { data, error } = await sb.rpc('get_detailed_reports');
    ok(error);
    return data;
  },
  ...
  async updateFinanceRecord(id, owed, paid) {
    const { error } = await sb.from('finances').update({ amount_owed: parseFloat(owed) || 0, amount_paid: parseFloat(paid) || 0 }).eq('id', id);
    ok(error);
    return true;
  },
  ...
  async markFinancePaid(id, paidBy, reference) {
    const { data } = await sb.from('finances').select('amount_owed').eq('id', id).single();
    if (!data) return false;
    const { error } = await sb.from('finances').update({ amount_paid: data.amount_owed, paid_by: String(paidBy).trim(), reference: String(reference).trim() }).eq('id', id);
    return !error;
  },

  async recordPaymentWithAmount(unit, totalOwed, totalPaid, actualPaid, paidBy, reference) {
    const { data, error } = await sb.rpc('record_payment_with_amount', {
  ...
  async recordMergedPayment(unitsSummary, totalPaid, paidBy, reference) {
    const { data, error } = await sb.rpc('record_merged_payment', {
  ...
  async getMyPaymentRequests(unit) {
    const { data, error } = await sb.from('payment_requests').select('*').eq('unit', unit.trim()).order('created_at', { ascending: false });
    ok(error);
    return (data || []).map(mapPaymentRequest);
  },

  async getPendingPaymentRequestsCount() {
    const { count, error } = await sb.from('payment_requests').select('*', { count: 'exact', head: true }).eq('status', 'pending');
    ok(error);
    return count || 0;
  },
  ...
  async approvePaymentRequest(id) {
    const { data, error } = await sb.rpc('approve_payment_request', { p_id: id });
    ok(error);
    return data;
  },

  async rejectPaymentRequest(id, reason) {
    const { error } = await sb.rpc('reject_payment_request', { p_id: id, p_reason: reason });
    ok(error);
    return true;
  },
  ...
  async getMaintenanceSummary(unit) {
    const [{ data: m }, { data: s }] = await Promise.all([
      sb.from('maintenance').select('paid_until_month').eq('unit', unit.trim()).maybeSingle(),
      sb.from('settings').select('value').eq('key', 'maint_fee').maybeSingle()
    ]);
  ...
  async setMaintenanceMonth(unit, newStr) {
    const { error } = await sb.from('maintenance').upsert({ unit: unit.trim(), paid_until_month: newStr });
    ok(error);
    return true;
  },

  async setMaintFee(f) {
    const { error } = await sb.from('settings').upsert({ key: 'maint_fee', value: String(f) });
    ok(error);
    return true;
  },

  async processMaintPayment(unit, monthsToAdd) {
    const { data, error } = await sb.rpc('process_maint_payment', { p_unit: unit, p_months: parseInt(monthsToAdd) });
    ok(error);
    return data;
  },
  ...
  async getMyTickets(unit) {
    const { data, error } = await sb.from('tickets').select('*').eq('unit', unit.trim());
    ok(error);
  ...
  async getOpenTicketsCount() {
    const { count, error } = await sb.from('tickets').select('*', { count: 'exact', head: true }).neq('status', 'resolved');
    ok(error);
    return count || 0;
  },
  ...
  async getAllAccessories() {
    const { data, error } = await sb.from('accessories').select('*').order('created_at', { ascending: false });
    ok(error);
  ...
  async requestAccessory(unit, accessoryId, quantity) {
    const { data, error } = await sb.rpc('request_accessory', { p_unit: unit, p_accessory_id: accessoryId, p_quantity: parseInt(quantity) });
    ok(error);
    return data;
  },

  async getMyAccessoryRequests(unit) {
    const { data, error } = await sb.from('accessory_requests').select('*').eq('unit', unit.trim()).order('created_at', { ascending: false });
    ok(error);
    return (data || []).map(mapAccessoryRequest);
  },

  async getAllAccessoryRequests() {
    const { data, error } = await sb.from('accessory_requests').select('*').order('created_at', { ascending: false });
    ok(error);
    return (data || []).map(mapAccessoryRequest);
  },

  async deliverAccessoryQuantity(id, deliveredQty) {
    const { data, error } = await sb.rpc('deliver_accessory_quantity', { p_id: id, p_delivered_qty: parseInt(deliveredQty) });
    ok(error);
    return data;
  },

  async deleteAccessoryRequest(id) {
    const { error } = await sb.from('accessory_requests').delete().eq('id', id);
    ok(error);
    return true;
  },

  async getAccessoriesParticipationReport() {
    const { data, error } = await sb.rpc('get_accessories_participation_report');
    ok(error);
